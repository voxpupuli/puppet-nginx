      ...
      describe 'os-independent items' do
            ...
            context "when #{param[:attr]} is #{param[:value]}" do
              ...
              it param[:title] do
                ...
                if matches.all? { |m| m.is_a? Regexp }
                  matches.each { |item| expect(subject).to contain_concat__fragment(fragment).with_content(item) }
                else
                ...
                Array(param[:notmatch]).each do |item|
                  expect(subject).to contain_concat__fragment("server1-500-#{Digest::MD5.hexdigest(params[:location].to_s)}").without_content(item)
                end
              end
            end
          end
        ...
        describe 'server_location_alias template content' do
          ...
          context 'location_alias template with default params' do
            ...
            it 'sets alias' do
              expect(subject).to contain_concat__fragment("server1-500-#{Digest::MD5.hexdigest('location')}").
        ...
        describe 'server_location_fastcgi template content' do
          ...
          context "when fastcgi_script is 'value'" do
            ...
            it 'sets fastcgi_script' do
              expect(subject).to contain_concat__fragment("server1-500-#{Digest::MD5.hexdigest(params[:location].to_s)}").
