...
describe 'nginx::service' do
  ...
  let :params do {
      ...
      :service_restart => '/etc/init.d/nginx configtest && /etc/init.d/nginx relaod',
  ...
  describe "when configtest_enable => true" do
    ...
    it { is_expected.to contain_service('nginx').with_restart('/etc/init.d/nginx configtest && /etc/init.d/nginx reload') }
