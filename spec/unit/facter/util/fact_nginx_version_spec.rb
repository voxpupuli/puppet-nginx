...
describe Facter::Util::Fact do
  ...
  context 'nginx' do
    context 'with current version output format' do
      before do
        allow(Facter::Util::Resolution).to receive(:which).with('nginx').twice { true } # rubocop:disable Lint/AmbiguousBlockAssociation
  ...
  context 'openresty' do
    context 'with current version output format' do
      before do
        allow(Facter::Util::Resolution).to receive(:which).with('nginx').twice { false } # rubocop:disable Lint/AmbiguousBlockAssociation
