...
define nginx::resource::upstream (
  ...
  $my_upstream = "${nginx::config::conf_dir}/conf.d/${name}-upstream.conf"
  ...
  if $ensure_real == 'absent' {
    ...
    # Concat < 1.1 doesn't have an ensure parameter, and 1.04 is latest
    # Puppet Labs-supported version... so use a File resource to remove
    # files we ask to be removed.
    file { $my_upstream :
      ensure => $ensure_real,
