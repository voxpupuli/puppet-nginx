...
#   [*listen_options*]      - Extra options for listen directive like 'default'
#   to catchall. Undef by default.
#   [*ipv6_enable*]         - BOOL value to enable/disable IPv6 support
#   (false|true). Module will check to see if IPv6
#                             support exists on your system before enabling.
#   [*ipv6_listen_ip*]      - Default IPv6 Address for NGINX to listen with this
#   vHost on. Defaults to all interfaces (::)
...
#   to catchall. Template will allways add ipv6only=on.
#                             While issue jfryman/puppet-nginx#30 is discussed,
#                             default value is 'default'.
#   [*index_files*]         - Default index files for NGINX to read when
#   traversing a directory
#   [*proxy*]               - Proxy server(s) for the root location to connect
#   to.  Accepts a single value, can be used in
...
#     rewrite www.domain.com to domain.com in order to avoid duplicate content
#     (SEO);
...
define nginx::resource::vhost (
  ...
  # Support location_cfg_prepend and location_cfg_append on default location
  # created by vhost
  ...
  if ($listen_port != $ssl_port) {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-699": content =>
      template('nginx/vhost/vhost_footer.erb'), }
  }
