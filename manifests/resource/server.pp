...
define nginx::resource::server (
  ...
  $server_enable_dir = "${nginx::config::conf_dir}/sites-enabled"
  ...
  # Support location_cfg_prepend and location_cfg_append on default location created by server
  ...
  if ($ssl == true) {
    ...
    # avoid the error when using wildcard cert on the multiple servers
