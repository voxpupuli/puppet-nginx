...
#  Extra options for listen directive like 'default_server' to catchall. Undef by default.
# @param listen_unix_socket_enable
#  BOOL value to enable/disable UNIX socket listening support (false|true).
# @param listen_unix_socket
#  Default unix socket for NGINX to listen with this server on. Defaults to UNIX /var/run/nginx.sock
...
# @param ipv6_enable
#  BOOL value to enable/disable IPv6 support (false|true). Module will check to see if IPv6 support
...
#  Pre-generated SSL Certificate file to reference for client verify SSL Support. This is not generated by
...
# @param ssl_key
#  Pre-generated SSL Key file to reference for SSL Support. This is not generated by this module. Set to
# @param ssl_port
#  Default IP Port for NGINX to listen with this SSL server on. Defaults to TCP 443
# @param ssl_prefer_server_ciphers
#  String: Specifies that server ciphers should be preferred over client ciphers when using the SSLv3 and
# @param ssl_protocols
#  SSL protocols enabled. Defaults to 'TLSv1 TLSv1.1 TLSv1.2'.
# @param ssl_buffer_size
#  Sets the size of the buffer used for sending data.
# @param ssl_ciphers
#  SSL ciphers enabled. Defaults to nginx::ssl_ciphers
# @param ssl_cache
# @param ssl_crl
...
#  String: When set, the stapled OCSP response will be taken from the specified file instead of querying
# @param ssl_stapling_responder
#  String: Overrides the URL of the OCSP responder specified in the Authority Information Access
...
#  String: Enables or disables session resumption through TLS session tickets.
# @param ssl_session_ticket_key
#  String: Sets a file with the secret key used to encrypt and decrypt TLS session tickets.
...
# @param proxy
#  Proxy server(s) for the root location to connect to.  Accepts a single value, can be used in
...
# @param raw_prepend
#  A single string, or an array of strings to prepend to the server directive (after cfg prepend
# @param raw_append
#  A single string, or an array of strings to append to the server directive (after cfg append
# @param location_raw_prepend
#  A single string, or an array of strings to prepend to the location directive (after custom_cfg
...
# @param access_log
#  Where to write access log (log format can be set with $format_log). This can be either a string or an
# @param error_log
#  Where to write error log. May add additional options like error level to the end. May set to 'absent',
# @param format_log
...
#  Allows one to set environemnt variables to pass to the backend application (Passenger 5.0+)
# @param passenger_pre_start
#  Allows setting a URL to pre-warm the host. Per Passenger docs, the "domain part of the URL" must match
# @param log_by_lua
#  Run the Lua source code inlined as the <lua-script-str> at the log request processing phase. This does
# @param log_by_lua_file
#  Equivalent to log_by_lua, except that the file specified by <path-to-lua-script-file> contains the Lua
...
#  Value to return when maintenance is on.  Default to return 503
# @param error_pages
#  Hash: setup errors pages, hash key is the http code and hash value the page
# @param locations
#  Hash of servers resources used by this server
# @param locations_defaults
#  Hash of location default settings
# @param add_listen_directive
#  Boolean to determine if we should add 'ssl on;' to the vhost or not. defaults to true for nginx 1.14 and older, otherwise false
