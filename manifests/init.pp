...
class nginx (
...

  {
    ...
    sites_available_mod => $sites_available_mod,
  }.each | $key, $val |{
    if ($val){
      $message = @("MESSAGE")
        [nginx] *** DEPRECATION WARNING***: HI! I notice that you are declaring some attributes in Class[nginx]:
        **attribute: $key
        **value: $val
        It is highly recommended to set these values via Hiera going forward. This will become mandatory in the near future.
        Please check out https://github.com/jfryman/puppet-nginx/blob/master/docs/hiera.md for more information.
        ------------------------------------------------------------------------
